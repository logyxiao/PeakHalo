...
应用图标由脚本生成，输出到 `Sources/PeakHalo/Resources/AppLogo.png`、`Sources/PeakHalo/Resources/AppIcon.icns` 和 `Assets/AppIcon.iconset`。
...
script                        构建运行脚本和图标生成脚本
```

## 说明

PeakHalo 当前是本地 macOS 应用项目，没有发布安装包。开发验证以 `./script/build_and_run.sh --verify` 为准。
...
The app icon is generated by script and written to `Sources/PeakHalo/Resources/AppLogo.png`, `Sources/PeakHalo/Resources/AppIcon.icns`, and `Assets/AppIcon.iconset`.

```bash
swift script/generate_app_icon.swift
```

After regenerating the icon, run:
...
PeakHalo is currently a local macOS app project without a packaged release. Use `./script/build_and_run.sh --verify` as the main development verification command.
