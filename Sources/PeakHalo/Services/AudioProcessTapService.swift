...
final class AudioProcessTapService {
    private struct ActiveTap {
        ...
        let renderState: AudioRenderState
    }
    ...
    private func createActiveTap(
        ...
        let sampleRate = nominalSampleRate(deviceID: aggregateID)
            ...
            ?? 48_000
        let rampCoefficient = AudioRenderPipeline.rampCoefficient(sampleRate: sampleRate)
        let renderState = AudioRenderState(
            volume: volume,
            isMuted: isMuted,
            boost: boost,
        ...
        status = AudioDeviceCreateIOProcIDWithBlock(&procID, aggregateID, queue) { _, inputData, _, outputData, _ in
            AudioRenderPipeline.render(inputData: inputData, outputData: outputData, state: renderState)
        }
